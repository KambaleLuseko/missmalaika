...
class GalleryProvider extends ChangeNotifier {
  List<GalleryModel> offlineData = [], latestData = [];

  save(
      {required GalleryModel data,
    ...
    response = await AppProviders.appProvider.httpPost(
        ...
        body: {"transaction": "addimage", ...data.toJson()});
    // print(response.body);
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      if (decoded['state'].toString().toLowerCase() == 'success') {
        ...
        ToastNotification.showToast(
            msg: "L'image a été enregistrée avec succès",
  ...
  getOnline({bool? isRefresh = false, String? value}) async {
    ...
    var response = await AppProviders.appProvider.httpPost(
        url: BaseUrl.getData,
        body: {"transaction": "getimages", "filter": value ?? 'latest'});
    List data = [];
    // print(response.body);
    ...
    List<GalleryModel> dataList = List<GalleryModel>.from(
  ...
  deleteImage({required GalleryModel data}) async {
    var response = await AppProviders.appProvider.httpPost(
        url: BaseUrl.saveData,
        body: {
